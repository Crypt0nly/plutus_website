...
// ── Orbit pill component ──────────────────────────────────────────────────────
function OrbitPill({ connector, angle, radius, duration, reverse, delay }) {
  return (
    <motion.div
      style={{
        position: 'absolute',
    ...
    >
      {/* Pill positioned at the top of the orbit circle, then rotated to starting angle */}
      <motion.div
        style={{
          position: 'absolute',
          top: 0,
      ...
      >
        <motion.div
          ...
          style={{
            ...
            border: `1px solid ${connector.color}35`,
            borderRadius: 999,
...
export default function Connectors() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (w < 480) setOrbitSize(130)
      else if (w < 768) setOrbitSize(170)
      else setOrbitSize(220)
    }
    update()
  ...
  // Distribute connectors across two orbit rings
  const innerRing = connectors.slice(0, 6)
  const outerRing = connectors.slice(6)

  const innerRadius = orbitSize
  const outerRadius = orbitSize + 90
        ...
        >
          ...
          {outerRing.map((c, i) => (
            <OrbitPill
              key={c.name}
              connector={c}
              angle={(360 / outerRing.length) * i + 25}
              radius={outerRadius}
              duration={48}
              reverse={true}
              delay={0}
        ...
        >
          ...
          <div style={{
            ...
            <p style={{ fontSize: 13, color: '#9a8abf', lineHeight: 1.6, margin: 0 }}>
              ...
              If it has an API, Plutus can use it. Define any endpoint once with the Custom API connector
              and control it with plain English.
            </p>
          </div>
        </motion.div>
